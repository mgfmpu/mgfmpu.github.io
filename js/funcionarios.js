...
async function loadData() {
    ...
    try {
        ...
        snap.forEach(d => {
            ...
            tr.innerHTML = `<td><strong>${f.Nome || obj.nome || '-'}</strong></td><td>${f.Funcao || obj.cargo || '-'}</td><td>${f.Telefone || obj.telefone || '-'}</td>
...
document.getElementById('dataForm').addEventListener('submit', async (e) => {
    ...
    try {
        if (currentEditId) {
            ...
            await updateDoc(docRef, { nome: document.getElementById('nome').value, cargo: document.getElementById('cargo').value, telefone: document.getElementById('telefone').value, endereco: document.getElementById('endereco').value });
        } else {
